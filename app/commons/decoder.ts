...
import { encodeFunctionData, type Hex, hexToBigInt } from 'viem'
import { z } from 'zod'
import type {
...
const decodeSafeMultiSendTransactionsParam = (bytes: string) => {
  ...
  try {
    ...
    let i = 0
    while (i < transactionsParam.length) {
      ...
      const dataEnd = dataLengthEnd + Number.parseInt(dataLength) * 2
      const _data = transactionsParam.slice(dataLengthEnd, dataEnd)
      if (Number.parseInt(dataLength) !== 0 && _data === '')
        throw new Error(
...
const decodeABIEncodedData = (calldata: string) => {
  ...
  try {
    ...
    console.error(error)
    throw new Error(`Failed to guess ABI encoded data for calldata ${calldata}`)
  }
}
  ...
  try {
    ...
    for (let i = 0; i < commandsBytes.length; i += 2) {
      ...
      if (command === undefined) {
        throw new Error('Failed to decode calldata as UniversalRouter commands')
      }
...
async function fetchFunctionFrom4Bytes({ selector }: { selector: string }) {
  try {
    const requestUrl = new URL('https://www.4byte.directory/api/v1/signatures/')
