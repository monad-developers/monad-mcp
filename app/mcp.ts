...
export const mcpHandler = initializeMcpApiHandler(
  (server) => {
    ...
    server.tool(
      ...
      async ({ address }) => {
        try {
          const response = await fetch(`https://api.blockvision.org/v2/monad/contract/source/code?address=${address}`, {
            method: 'GET',
            headers: {
    ...
    server.tool(
      ...
      async () => {
        try {
          ...
          if (!response.ok) {
            return {
              content: [{ type: "text", text: `Error fetching Monad docs: ${response.status}` }],
            };
          }
          const text = await response.text();
          // Add base URL to links
          const baseUrl = "https://docs.monad.xyz/";
          const processedText = text.replace(/\[([^\]]+)\]\(([^)]+)\)/g, (match, title, path) => {
          ...
          return {
            content: [{ type: "text", text: `Error fetching Monad docs: ${error instanceof Error ? error.message : String(error)}` }],
          };
        }
      }
    );
      ...
      const results = operations.map((operation) => {
        try {
          switch (operation) {
            ...
            case "toNumber":
              if (isHex(currentValue)) {
                return { operation, result: Number(hexToBigInt(currentValue)) };
              }
            ...
            case "toKeccak256":
              if (isHex(currentValue)) {
                ...
                return { operation, result: toKeccak256(stringToHex(currentValue)) };
              }
            ...
            case "toBigInt":
              if (isHex(currentValue)) {
                return { operation, result: hexToBigInt(currentValue).toString() };
              }
...
server.tool(
  ...
  "Decode Ethereum transaction calldata and get function information",
  {
    input: z.object({
      ...
      tx: z.string().optional().describe("Transaction hash or explorer URL"),
      chainId: z.string().optional().describe("Chain ID (required if tx is a hash)")
  ...
  async ({ input }) => {
    try {
      ...
      if (input.tx) {
        try {
          ...
          if (/^0x([A-Fa-f0-9]{64})$/.test(input.tx)) {
            ...
            if (!input.chainId) throw new Error("Chain ID required when using transaction hash");
            ...
            const chainKey = Object.keys(c as any).filter((chainKey) => {
              ...
              const explorerDomainDefault = chain.blockExplorers.default.url.split("//").pop()!;
              const explorerDomainEtherscan = chain.blockExplorers.etherscan?.url.split("//").pop();
              
              return input.tx!.split("/").some(urlPart => 
                urlPart.toLowerCase() === explorerDomainDefault.toLowerCase() ||
                (explorerDomainEtherscan && urlPart.toLowerCase() === explorerDomainEtherscan.toLowerCase())
              );
            })[0];
            
            if (!chainKey) throw new Error("Could not determine chain from explorer URL");
          ...
          throw new Error(`Failed to fetch transaction data: ${error instanceof Error ? error.message : String(error)}`);
        }
      }
          ...
          content: [
            {
              ...
              text: JSON.stringify({
                ...
                message: "Could not find matching function signature for this selector"
              }, null, 2)
            }
          ]
        };
      ...
      const parametersString = functionInterface.slice(functionInterface.indexOf("("));
